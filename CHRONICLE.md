...
|----------|------|----------|-------|
| Info | cmd/ralph coverage ceiling at 71.8% | `cmd/ralph/wiring.go` — `runWithRegentTUI`, `finishTUI`, `runWithTUIAndState`; `cmd/ralph/main.go` — `main`; `cmd/ralph/quit_unix.go`/`quit_windows.go` — `registerQuitHandler` | These functions require a real TTY (bubbletea) or are OS-level signal handlers. Not actionable without a bubbletea headless test mode. `internal/loop` runner.Run at 0% on Windows (shell script tests skipped). Added `specNewCmd` existing-spec error path and `specListCmd` not-dir error path (Unix-only) in v0.0.59. `spec.New` MkdirAll error path and `ScaffoldProject` `.gitignore`-is-dir read error path added in v0.0.60; `internal/spec` 94.9%→96.2%, `internal/config` 91.3%→92.1%. Added `initCmd` ScaffoldProject error path in v0.0.61; `initCmd` 77.8%→88.9%, cmd/ralph 71.5%→71.8%. Remaining partial-coverage gaps (git.Stash "No local changes" path, regent.SaveState marshal/createTemp/write/close errors) are not reliably testable on modern git/cross-platform. |
